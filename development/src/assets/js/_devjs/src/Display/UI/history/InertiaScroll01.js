...
export default class InertiaScroll {
  ...
  ready() {
    ...
    this.tit01ST = this.$tit01.offset().top - gb.in.r.ws.h/4*3; this.tit01STLock = false;
    this.tit02ST = this.$tit02.offset().top - gb.in.r.ws.h/4*3; this.tit02STLock = false;
    this.text02ST = this.$text02.offset().top - gb.in.r.ws.h/4*3; this.text02STLock = false;
    this.text03ST = this.$text03.offset().top - gb.in.r.ws.h/4*3; this.text03STLock = false;
    this.text04ST = this.$text04.offset().top - gb.in.r.ws.h/4*3; this.text04STLock = false;
    this.text05ST = this.$text05.offset().top - gb.in.r.ws.h/4*3; this.text05STLock = false;
  ...
  run() {
    ...
    // ??ӳ
    // inner
    TweenMax.set(this.$wrapper,{y: -this.wrapST * gb.in.Layout.size.scale.rate})
    // target   
    var self = this; 
    this.$target.each(function (i) {

      TweenMax.set($(this),{y: gb.in.Layout.size.scale.rate * self.dirList[i] * (-self.targetST * Number(self.rateList[i]) + self.marginList[i])})
  ...
  showContents() {
    ...
    if (this.text04ST < this.wrapST && !this.text04STLock) {
      this.text04STLock = true;

      var opt = {$target: this.$text04,dur: 1.5,delay: 0,blur: 10,blurTarget: 0,ease: Power2.easeOut};
      this.libs.blur(opt);

      TweenMax.to(this.$text04, 1.5, {y: 0,x: '-50%',z:1, opacity:1,ease: Power2.easeOut});
  ...
  onResize() {
    ...
    this.tit01ST = this.$tit01.offset().top - gb.in.r.ws.h/4*3;
    this.tit02ST = this.$tit02.offset().top - gb.in.r.ws.h/4*3;
    this.text02ST = this.$text02.offset().top - gb.in.r.ws.h/4*3;
    this.text03ST = this.$text03.offset().top - gb.in.r.ws.h/4*3;
    this.text04ST = this.$text04.offset().top - gb.in.r.ws.h/4*3;
    this.text05ST = this.$text05.offset().top - gb.in.r.ws.h/4*3;
